...
use App\Mail\LuaranDitolakMail;
...
class BerkasController extends Controller
{
    ...
    $kkn = $kknQuery->paginate(10);
        ...
        return view('pages.admin.berkas.index', compact('skema', 'kkn', 'search', 'selectedSkema'));
    }
    ...
    public function mahasiswa(Request $request, string $nama, string $periode)
{
    ...
    // dd($mahasiswaList->toArray());
        ...
        return view('pages.admin.berkas.list-mahasiswa', compact('kkn', 'mahasiswaList', 'prodi', 'search', 'selectedProdi'));
...
private function getFilteredMahasiswaList(KknModel $kkn, $search, $selectedProdi)
{
    ...
    public function berkas(string $nama, string $periode, string $nim)
    {
        ...
        $berkas = BerkasKknModel::whereIn('luaran_id', $luaranIds)->get();
            ...
            return redirect()->route('list.berkas.kkn')->with('error_message_not_found', 'Data kkn tidak ditemukan');
        }
        return view('pages.admin.berkas.berkas', compact('kkn', 'berkas'));
    }
    ...
    public function editStatusBerkas(Request $request, string $nama, string $periode, string $nim, string $berkasId)
{
    try {
        ...
        $namaMahasiswa = $mahasiswa[0]['mahasiswa']['nama'];

        $userFromMahasiswa = $mahasiswa[0]['mahasiswa']['user'];

        $mahasiswaEmail = $userFromMahasiswa['email'];
        ...
        $statusBerkas = BerkasKknModel::where('berkas_id', $berkasId)->first();

        if ($statusBerkas->status == "Ditolak") {
            Mail::to($mahasiswaEmail)->send(new LuaranDitolakMail($statusBerkas, $namaMahasiswa));
        }
