...
class StaffKabupatenController extends Controller
{
    public function index()
    {
        $kabupaten = KabupatenModel::all();
        return view('pages.staff.kabupaten.index', compact('kabupaten'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('daerah-kabupaten.index');
    ...
    public function update(Request $request, string $id)
    {
        try {
            ...
            return redirect()->route('daerah-kabupaten.index')->with('error_message_not_found', 'Data kabupaten tidak ditemukan');
        }
        $data = $request->all();
    ...
    public function destroy(string $id)
    {
        try {
            ...
            if ($kabupaten->kecamatans()->count() > 0) {
                ...
                return redirect()->route('daerah-kabupaten.index')->with('error_message_delete', $errorMessage);
            }
            $kabupaten->delete();

            return redirect()->route('daerah-kabupaten.index')->with('success_message_delete', 'Data kabupaten berhasil dihapus');
        } catch (ModelNotFoundException $e) {
            return redirect()->route('daerah-kabupaten.index')->with('error_message_not_found', 'Data kabupaten tidak ditemukan');
        } catch (QueryException $e) {
            if ($e->getCode() === '23000') {
