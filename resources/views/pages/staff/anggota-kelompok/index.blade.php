                      ...
                      <div class="row h-100">
                        ...
                        <div class="col-sm-7">
                          <div class="card-body text-sm-end text-center ps-sm-0">
                            <a href="{{ route('staff.create.anggota.kelompok', ['nama' => $kkn->nama, 'periode' => $kkn->periode->nama, 'kelompok_id' => request()->route('kelompok_id')]) }}"
                                class="btn btn-primary mb-2 text-nowrap add-new-role">
                               <small>Tambah Anggota</small>
                            </a>
                            <small class="mb-0 mt-1">Tambah anggota kelompok</small>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <form class="card-body" action="{{ route('staff.edit.dpl.kelompok', ['nama' => $kkn->nama, 'periode' => $kkn->periode->nama, 'kelompok_id' => request()->route('kelompok_id')]) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                  <div class="col-md-12">
                    <div class="card mb-4">
                        <div class="card-body demo-vertical-spacing demo-only-element">
                            <div class="row mb-2">
                                <div class="col-sm-10">
                                    <select id="select2Basic" name="dpl_id" class="select2-icons form-select">
                                        @foreach($dpl as $data)
                                            @php
                                                $isSelected = false;
                                                foreach ($kkn->kknmahasiswas as $kknmahasiswa) {
                                                    if (old('dpl_id', $kknmahasiswa['dpl_id']) == $data->dpl_id) {
                                                        $isSelected = true;
                                                        break;
                                                    }
                                                }
                                            @endphp
                                            <option value="{{ $data->dpl_id }}" @if ($isSelected) selected @endif data-icon="ti ti-user">{{ $data->nama }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-2">
                                    <button type="submit" class="btn btn-primary mx-1">Ubah Dpl
                                    </button>
                                </div>
                            </div>
                        </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-inline-block">
                                        ...
                                        <button  data-bs-toggle="modal"
                                        data-bs-target="#onboardHorizontalImageModal{{ $data->mahasiswa_id }}"
                                        class="dropdown-item delete-record"><i class="ti ti-trash me-1"></i>Delete</button>
                                        </div>
                                        </div>
                                </td>
                            </tr>
...
id="onboardHorizontalImageModal{{ $data->mahasiswa_id }}"
tabindex="-1"
aria-hidden="true"
>
    ...
    <div class="modal-body onboarding-horizontal p-0">
      ...
      <div class="onboarding-content mb-0">
        ...
        <small class="onboarding-info">
          Dengan menghapus {{ $data->nama }}, peserta ini akan dihapus kelompok ini.
        </small>
      </div>
    </div>
    <form method="POST" action="{{ route('staff.delete.anggota.kelompok', ['nama' => $kkn->nama, 'periode' => $kkn->periode->nama, 'kelompok_id' => request()->route('kelompok_id'), 'mahasiswa_id' => $data->mahasiswa_id]) }}">
      @csrf
      @method('DELETE')
