        ...
        <div class="col-lg-12 mb-4 order-0">
            <div class="card">
              <div class="d-flex align-items-end row">
                <div class="col-sm-7">
                  <div class="card-body">
                    <h5 class="text-primary">Edit Mahasiswa</h5>
                    <p>Di sini anda dapat menambahkan user sebagai Mahasiswa, tolong isi form dengan benar</p>
         ...
         <div class="card mb-4">
            <form class="card-body" action="{{ route('kuliah-kerja-nyata.update', $kkn->kkn_id) }}" method="post" enctype="multipart/form-data">                @csrf
                @csrf
                @method('PUT')
                    ...
                    <div class="row mb-3">
                      ...
                      <div class="col-sm-10">
                        <div class="input-group input-group-merge">
                          ...
                          ></span>
                          <input type="text" class="form-control" placeholder="YYYY-MM-DD" id="flatpickr-date" name="tanggal_mulai" value="{{ $kkn->tanggal_mulai }}" />
                        </div>
                      </div>
                    </div>
                    <div class="row mb-3">
                      ...
                      <div class="col-sm-10">
                          <div class="input-group input-group-merge">
                              ...
                              ></span>
                              <input type="text" class="form-control" placeholder="YYYY-MM-DD" id="flatpickr-date2" name="tanggal_selesai" value="{{ $kkn->tanggal_selesai }}" />
                            </div>
                      </div>
                    </div>
                </div>
                      ...
                      <div class="col-sm-10">
                          <select id="select2Icons" name="desa_id" class="select2-icons form-select">
                              @foreach($desa as $data)
                                  <option value="{{ $data->desa_id }}" @if (old('desa', $kkn->desa->desa_id) == $data->desa_id)
                                    selected @endif data-icon="ti ti-map-pin">Desa {{ $data->nama }}
                      ...
                      <div class="col-sm-10">
                          <select id="select2Basic" name="periode_id" class="select2-icons form-select">
                              @foreach($periode as $data)
                                  <option value="{{ $data->periode_id }}" @if (old('periode', $kkn->periode->periode_id) == $data->periode_id)
                                    selected @endif data-icon="ti ti-calendar-event">{{ $data->nama }}</option>
                              @endforeach
                            </select>
                      </div>
                    </div>
                      ...
                      <div class="col-sm-10">
                          <select id="select2Icons2" name="skema_id" class="select2-icons form-select">
                              @foreach($skema as $data)
                                  <option value="{{ $data->skema_id }}" @if (old('skema', $kkn->skema->skema_id) == $data->skema_id)
                                    selected @endif data-icon="ti ti-category-2">{{ $data->nama }}</option>
                              @endforeach
                            </select>
                      </div>
                    </div>
                      ...
                      <div class="col-sm-10">
                          <div class="input-group input-group-merge">
                              <span id="basic-icon-default-phone2" class="input-group-text"
                                ><i class="ti ti-calendar-minus"></i
                              ></span>
                              <input type="text" class="form-control" placeholder="YYYY-MM-DD" id="flatpickr-date3" name="tanggal_pendaftaran" value="{{ $kkn->tanggal_pendaftaran }}" />
                            </div>
                      </div>
                    </div>
                    <div class="row mb-3">
                      ...
                      <div class="col-sm-10">
                        <select id="select2Icons3" name="status" class="select2-icons form-select">
                            <option value="Belum Dimulai" {{ $kkn->status === 'Belum Dimulai' ? 'selected' : '' }} data-icon="ti ti-api-app" selected>Belum Dimulai</option>
                            <option value="Sedang Berlangsung" {{ $kkn->status === 'Sedang Berlangsung' ? 'selected' : '' }} data-icon="ti ti-api-app">Sedang Berlangsung</option>
                            <option value="Selesai" {{ $kkn->status === 'Selesai' ? 'selected' : '' }} data-icon="ti ti-api-app">Selesai</option>
                          </select>
                      </div>
                    </div>
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10" id="form-container">
                            @foreach($persyaratan as $index => $p)
...
<script>
    ...
    // Menghitung jumlah input yang ada saat halaman dimuat
    let formCount = document.querySelectorAll('input[name^="nama_persyaratan"]').length;
